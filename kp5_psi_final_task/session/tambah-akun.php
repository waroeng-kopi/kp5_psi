    ...
    <section class="hero d-flex align-items-center justify-content-center text-center" id="hero" style="top: 80px;">
      <form method="post" class="d-flex mx-auto my-auto flex-column align-items-center justify-content-center text-center" style="width: 94vw; min-height: 100vh;">
        <div class="d-flex flex-column border border-dark p-5 pt-4 pb-4 rounded-4 gap-2">
          ...
          <h3><b>Membuat Akun Baru</b></h3>
          <?php 
            $p = mysqli_query($connection,'SELECT * FROM user');
            $jumlah =  mysqli_num_rows($p); 
          ?>
          <input type="hidden" name="userid" value="<?php echo $jumlah+1 ?>">
          ...
          <div class="w-auto mx-auto my-auto d-flex justify-content-between" style="min-width: 300px">
            ...
            <input type="text" name="tipeAkun">
          </div>
          <div class="w-auto mx-auto my-auto d-flex justify-content-between" style="min-width: 300px">
            <label for="saldoAwal">saldoAwal : </label>
            <input type="text" name="saldoAwal">
          </div>
          <input type="submit" name="submit" value="Tambahkan" class="d-flex mx-3 my-auto mt-3 px-5 py-2 bg-primary text-white rounded-4 border-0">
        </div>
      </form>
      <?php 
        ...
        if (isset($_POST['submit'])) {
          ...
          $connection->query("INSERT INTO user (userId, nama, deskripsi, tipeAkun, saldoAwal) VALUES ('$userId','$nama','$deskripsi','$tipeAkun','$saldoAwal') ");

          echo "<script>alert('Pendaftaran Sukses, Silakan Akses'); </script>";
